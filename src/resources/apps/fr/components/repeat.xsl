...
<xsl:stylesheet version="2.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template
        ...
        <xsl:variable name="tokenized-path"
            select="tokenize(if (@nodeset) then @nodeset else @ref, '/')"/>
        ...
        <xsl:variable name="repeat-expression"
            select="if (@nodeset) then @nodeset else if (@ref) then @ref else concat('xxforms:bind(''', @bind, ''')')"
            as="xs:string"/>
        ...
        <xhtml:div class="yui-dt">
            <xhtml:div class="yui-dt-hd">
                ...
                <xhtml:table
                    class="yui-dt-table fr-repeat {if ($is-table-appearance) then 'fr-repeat-table' else 'fr-repeat-sections'} fr-grid {if (@columns) then concat('fr-grid-', @columns, '-columns') else ()}">
                    <xhtml:thead class="yui-dt-hd">
                        ...
                        <xhtml:tr class="fr-dt-master-row">
                                        ...
                                        <xhtml:div class="yui-dt-liner">
                                            ...
                                            <xforms:trigger appearance="minimal" ref=".[$can-add]">
                                                ...
                                                <xforms:insert ev:event="DOMActivate"
                                                  ...
                                                  context="."
                                                  nodeset="{if (@after) then @after else if (@nodeset) then @nodeset else '$fr-repeat-sequence'}"
                                                  at="index('{$fr-repeat/@id}')"/>
                                            </xforms:trigger>
                                            <xforms:group ref=".[not($can-add)]">
                                                <xhtml:img width="16" height="16"
                                                  src="/apps/fr/style/images/pixelmixer/plus_16g.png"
                                                  alt=""/>
                                            </xforms:group>
                                        </xhtml:div>
                                    </xhtml:th>
                            ...
                            <xsl:for-each
                                select="(fr:body | xhtml:body)/(xhtml:tr[1] | fr:tr[1])/(xhtml:td | fr:td)/*[1]">
                                <xhtml:th>
                                    <xhtml:div class="yui-dt-liner">
                                        <xforms:group>
                    ...
                    <xhtml:tbody class="yui-dt-data">
                        ...
                        <xsl:for-each select="fr:body | xhtml:body">
                            ...
                            <xforms:repeat id="{$fr-repeat/@id}" ref="{$repeat-expression}">
                                ...
                                <xhtml:tr
                                    class="{{string-join((if ($repeat-position mod 2 = 1) then 'yui-dt-even' else 'yui-dt-odd', if ($repeat-position = 1) then 'yui-dt-first' else ()), ' ')}}">
                                    <!--<xsl:if test="$has-origin and not($readonly)">-->
                                    ...
                                    <xsl:if test="$has-origin and not($readonly)">
                                        <xforms:group ref=".[not(exforms:readonly(.))]">
                                            <xhtml:td
                                                class="fr-repeat-column fr-repeat-column-trigger">
                                                <xhtml:div class="yui-dt-liner">
                                                  <!-- Remove trigger -->
                                                  ...
                                                  <xhtml:img width="16" height="16"
                                                  src="/apps/fr/style/images/pixelmixer/delete_16g.png"
                                                  alt=""/>
                                                  </xforms:group>
                                                </xhtml:div>
                                            </xhtml:td>
                                        </xforms:group>
                                        <!-- STANFORD HACK: This fixes repeat elements when they are READONLY. Otherwise, the cell with the delete button is left out and 
                                            the grid gets shifted over. -->
                                        <xforms:group ref=".[exforms:readonly(.)]">

                                            <xhtml:td class="fr-grid-td">
                                                <xhtml:div class="yui-dt-liner"> </xhtml:div>
                                            </xhtml:td>
                                   ...
                                   <xsl:apply-templates
                                        select="(xhtml:tr[1] | fr:tr[1])/(xhtml:td | fr:td)"/> 
                            ...
                            <!-- IE display HACK -->
                            <xhtml:tr class="fr-repeat-last-line">
                                <xhtml:td/>
                            </xhtml:tr>
                        </xsl:for-each>
                    </xhtml:tbody>
